...
def outputHandler():    
    if (os.path.exists(args.output)):
        ...
        for line in fout:
            ...
            seen[pieces[0]]=1
        fout.close()
        fout=open(args.output, 'a') #WAS 'a' not 'w'
    else:
...
def imageWriter(imgfile,images):
...
def testMain():     #main function with log message and time captures.
    outfileName = "DZImages_Map_log"+t.strftime("_%a-%d-%b-%H:%M:%S")+".txt"
    log = []
    ...
    global stats, fft, lap, edge, feats 
    ...
    for directory in images:    
        ...
        for file in tmpList:
            ...
            #the image writer function takes arguments in the form of 'subdirectory/file.extension'
    ...
    ret = pool.map(imageWriter_map_revision,imagesSubDirectories)  
    ...
    diff= end-start
    msg = "Image info writing took: "+str(diff)+" seconds."
    log.append(msg)
    msg = "Used "+str(numProcesses)+" paralell processes."
    log.append(msg)
